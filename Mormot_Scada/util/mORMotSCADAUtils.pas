...
unit mORMotSCADAUtils;

interface

uses System.SysUtils, System.StrUtils, Variants;

type TEditDisplayEncoding = (deDefault, deBoolean, deDecimal, deHex, deTime);

function StrToFloatLocale(const S: string): Extended;overload;
function StrToFloatLocale(const V: Variant): Extended;overload;
...
var xS: string;
begin
  if VarIsNull(V) OR VarIsClear(V) OR VarIsEmpty(V) then
    xS:= '0'
  else
    xS:= V;
  Result:= StrToFloatLocale(xS);
end;
